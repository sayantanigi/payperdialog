...
function availTime(start_date, from_time, end_date, to_time, bookingTime) {
    ...
    $.each(timesArray,function(i) {
        if(bookingTime.indexOf(timesArray[i]) != -1) {
            $('.bookingcontent').append('<tr><td class="pastHours"><input type="checkbox" class="pasthours" id="pastHours_'+timesArray[i].replace(':00', '').trim()+'" name="pastHours[]" value="'+timesArray[i]+'" required checked onclick="removeSlot('+timesArray[i].replace(':00', '').trim()+')">'+timesArray[i]+'</td></tr>');
        } else {
            $('.bookingcontent').append('<tr><td class="pastHours"><input type="checkbox" class="pasthours" id="pastHours_'+timesArray[i].replace(':00', '').trim()+'" name="pastHours[]" value="'+timesArray[i]+'" required onclick="removeSlot('+timesArray[i].replace(':00', '').trim()+')">'+timesArray[i]+'</td></tr>');
        }
    });
}
